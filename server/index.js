...
// Special handler for airdrop detail pages
app.get('/airdrops/:id', async (req, res, next) => {
  const id = req.params.id;
  console.log(`Handling client-side route for airdrop detail: ${id}`);

  try {
    // Check if the airdrop exists in the database
    let airdrop = null;

    // Check if the ID is a valid MongoDB ObjectID
    if (mongoose.Types.ObjectId.isValid(id)) {
      console.log(`Checking if airdrop exists with _id: ${id}`);
      airdrop = await Airdrop.findById(id);
    }

    // If not found, try to find by airdropId
    if (!airdrop) {
      console.log(`Checking if airdrop exists with airdropId: ${id}`);
      airdrop = await Airdrop.findOne({ airdropId: id });
    }

    if (airdrop) {
      console.log(`Airdrop exists: ${airdrop.title}, serving client-side route`);
    } else {
      console.log(`Airdrop does not exist with ID: ${id}, but still serving client-side route`);
    }
  } catch (error) {
    console.error(`Error checking airdrop existence: ${error.message}`);
  }
...
// Handle other client-side routes before the API 404 handler
